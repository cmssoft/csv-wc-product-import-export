...
class CSV_WC_AJAX_Handler {
	...
	*/
	public function __construct() {
		add_action( 'wp_ajax_csv_wc_import_request', array( $this, 'csv_import_request' ) );
		add_action( 'wp_ajax_csv_wc_import_regenerate_thumbnail', array( $this, 'regenerate_thumbnail' ) );
		add_action( 'wp_ajax_csv_wc_import_validation', array( $this, 'csv_wc_import_validation' ) );
		add_action( 'wp_ajax_csv_wc_cron_list', array( $this, 'csv_wc_cron_list' ) );
		add_action( 'wp_ajax_csv_wc_filelog_list', array( $this, 'csv_wc_filelog_list' ) );
		add_action( 'wp_ajax_csv_wc_datalog_list', array( $this, 'csv_wc_datalog_list' ) );
		add_action( 'wp_ajax_csv_wc_cron_status', array( $this, 'csv_wc_cron_status' ) );
		add_action( 'wp_ajax_csv_wc_cron_delete', array( $this, 'csv_wc_cron_delete' ) );
		add_action( 'wp_ajax_csv_wc_log_delete', array( $this, 'csv_wc_log_delete' ) );
	}
	...
	*/
	function csv_wc_cron_list(){
		...
		$sort['name'] = (isset($_POST['sort']) && $_POST['sort']!="") ? $_POST['sort'] : 'created_at';
		$sort['order'] = (isset($_POST['order']) && $_POST['order']!="") ? $_POST['order'] : 'DESC';
		...
		$link = 'admin.php?page=csv_wc_import_export&tab=cron';
	...
	*/
	function csv_wc_filelog_list(){
		...
		// sort by row column or created
		$sort['name'] = (isset($_POST['sort']) && $_POST['sort']!="") ? $_POST['sort'] : 'file_date';
		$sort['order'] = (isset($_POST['order']) && $_POST['order']!="") ? $_POST['order'] : 'DESC';
		$sort_by = ' ORDER BY ' . $sort['name'] . ' '. $sort['order'];
		...
		// initial link for pagination.
		// "page" must be the  menu slug / clean url from the add_menu_page
		$link = 'admin.php?page=csv_wc_import_export&tab=logs';
		$view_link = 'admin.php?page=csv_wc_import_export&tab=logs';
	...
	*/
	function csv_wc_datalog_list(){
		global $wpdb;
     	
     	//log file
     	$filename = isset($_POST['log_file']) ? $_POST['log_file'] : '';
        ...
        if ($filename) {
            ...
            // initial link for pagination.
            // "page" must be the  menu slug / clean url from the add_menu_page
            $link = 'admin.php?page=csv_wc_import_export&tab=logs&log_file='.$filename;
	...
	*/
	public function csv_wc_import_validation(){
		global $wpdb;
        $max_filesize = (int)(ini_get("upload_max_filesize"));
        $file = $_FILES['file'];
        $extension = 'csv';
		$is_cron = trim($_POST['is_cron']);
