	...
	public function process_product( $post, $imported_file='' ) {
		...
		try{
			...
			if ( empty( $post['post_parent'] ) ) {
				$this->add_import_result( 'skipped', __( 'No product variation parent set', CWPIE_TRANSLATE_NAME ), $processing_product_id, 'Not set', $post['sku'] );
			...
			if ( ! empty( $processing_product_id ) && isset( $this->processed_posts[ $processing_product_id ] ) ) {
				$this->add_import_result( 'skipped', __( 'Product variation already processed', CWPIE_TRANSLATE_NAME ), $processing_product_id, get_the_title( $post['post_parent'] ), $post['sku'] );
			...
			if ( isset( $post['post_status'] ) && 'auto-draft' === $post['post_status'] ) {
				$this->add_import_result( 'skipped', __( 'Skipping auto-draft', CWPIE_TRANSLATE_NAME ), $processing_product_id, get_the_title( $post['post_parent'] ), $post['sku'] );
			...
			if ( ! $post_parent_exists ) {
				$this->add_import_result( 'failed', __( 'Variation parent does not exist', CWPIE_TRANSLATE_NAME ), $processing_product_id, 'Does not exist', $post['sku'] );
			...
			if ( $merging && get_post_type( $processing_product_id ) !== 'product_variation' ) {
				$this->add_import_result( 'skipped', __( 'Post is not a product variation', CWPIE_TRANSLATE_NAME ), $processing_product_id, 'Not a variation', $post['sku'] );
			...
			if ( $merging ) {
				...
				if ( $this->variation_exists( $post_parent, $processing_product_id, $processing_product_sku ) ) {
					$this->add_import_result( 'skipped', __( 'Variation already exists', CWPIE_TRANSLATE_NAME ), $processing_product_id, get_the_title( $post['post_parent'] ), $processing_product_sku );
				...
				if ( is_wp_error( $post_id ) ) {
					...
					$this->add_import_result( 'failed', __( 'Failed to import product variation', CWPIE_TRANSLATE_NAME ), $processing_product_id, get_the_title( $post['post_parent'] ), $post['sku'] );
