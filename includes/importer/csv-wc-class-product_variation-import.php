...
class CSV_WC_Product_Variation_Import extends CSV_WC_Product_Import {
	...
	*/
	public function __construct() {
		parent::__construct();
		$this->import_page = 'csv_wc_variation';
	...
	public function process_product( $post, $imported_file='' ) {
		...
		try{
			...
			if ( empty( $post['post_parent'] ) ) {
				$this->add_import_result( 'skipped', __( 'No product variation parent set', CSV_TRANSLATE_NAME ), $processing_product_id, 'Not set', $post['sku'] );
			...
			if ( ! empty( $processing_product_id ) && isset( $this->processed_posts[ $processing_product_id ] ) ) {
				$this->add_import_result( 'skipped', __( 'Product variation already processed', CSV_TRANSLATE_NAME ), $processing_product_id, get_the_title( $post['post_parent'] ), $post['sku'] );
			...
			if ( isset( $post['post_status'] ) && 'auto-draft' === $post['post_status'] ) {
				$this->add_import_result( 'skipped', __( 'Skipping auto-draft', CSV_TRANSLATE_NAME ), $processing_product_id, get_the_title( $post['post_parent'] ), $post['sku'] );
			...
			if ( ! $post_parent_exists ) {
				$this->add_import_result( 'failed', __( 'Variation parent does not exist', CSV_TRANSLATE_NAME ), $processing_product_id, 'Does not exist', $post['sku'] );
			...
			if ( $merging && get_post_type( $processing_product_id ) !== 'product_variation' ) {
				$this->add_import_result( 'skipped', __( 'Post is not a product variation', CSV_TRANSLATE_NAME ), $processing_product_id, 'Not a variation', $post['sku'] );
			...
			if ( $merging ) {
				...
				if ( $this->variation_exists( $post_parent, $processing_product_id, $processing_product_sku ) ) {
					$this->add_import_result( 'skipped', __( 'Variation already exists', CSV_TRANSLATE_NAME ), $processing_product_id, get_the_title( $post['post_parent'] ), $processing_product_sku );
				...
				if ( is_wp_error( $post_id ) ) {
					...
					$this->add_import_result( 'failed', __( 'Failed to import product variation', CSV_TRANSLATE_NAME ), $processing_product_id, get_the_title( $post['post_parent'] ), $post['sku'] );
	...
	*/
	public function greet() {
		$action     = 'admin.php?import=csv_wc_variation&amp;step=1&amp;merge=' . ( ! empty( $_GET['merge'] ) ? 1 : 0 );
