	...
	*/
	public function process_product( $post, $merge_empty_cells=0, $imported_file='' ) {
		...
		if ( ! empty( $processing_product_id ) && isset( $processed_posts[ $processing_product_id ] ) ) {
			CWPIE_Product_Import::add_import_result( 'skipped', __( 'Product already processed', CWPIE_TRANSLATE_NAME ), $processing_product_id, $processing_product_title, $processing_product_sku );
		...
		if ( ! empty ( $post['post_status'] ) && $post['post_status'] == 'auto-draft' ) {
			CWPIE_Product_Import::add_import_result( 'skipped', __( 'Skipping auto-draft', CWPIE_TRANSLATE_NAME ), $processing_product_id, $processing_product_title, $processing_product_sku );
		...
		if ( ! $merging ) {
			if ( CWPIE_Product_Import::product_exists( $processing_product_title, $processing_product_sku, $post['post_name'] ) ) {
				CWPIE_Product_Import::add_import_result( 'skipped', __( 'Product already exists', CWPIE_TRANSLATE_NAME ), $processing_product_id, $processing_product_title, $processing_product_sku );
			...
			if ( $processing_product_id && is_string( get_post_status( $processing_product_id ) ) ) {
				CWPIE_Product_Import::add_import_result( 'skipped', __( 'Importing post ID conflicts with an existing post ID', CWPIE_TRANSLATE_NAME ), $processing_product_id, get_the_title( $processing_product_id ), '' );
		...
		if ( $merging && $processing_product_id && get_post_type( $processing_product_id ) !== 'product' ) {
			CWPIE_Product_Import::add_import_result( 'skipped', __( 'Post is not a product', CWPIE_TRANSLATE_NAME ), $processing_product_id, $processing_product_title, $processing_product_sku );
		...
		if ( $merging ) {
			...
			if ( sizeof( $postdata ) > 1 ) {
				...
				if ( is_wp_error( $result ) ) {
					...
					CWPIE_Product_Import::add_import_result( 'failed', implode( ', ', $messages ), $post_id, $processing_product_title, $processing_product_sku );
			...
			if ( is_wp_error( $post_id ) ) {
				CWPIE_Product_Import::add_import_result( 'failed', __( 'Failed to import product', CWPIE_TRANSLATE_NAME ), $processing_product_id, $processing_product_title, $processing_product_sku );
				...
				if ( version_compare( WC_VERSION, '3.0', '>=' ) ) {
					CWPIE_Product_Import::set_catalog_visibility( $post_id, $post );
					CWPIE_Product_Import::set_featured( $post_id, $post );
				}
		...
		CWPIE_Product_Import::process_terms( $post_id, $post['terms'] );
		...
		if ( ! empty( $post['images'] ) && is_array( $post['images'] ) ) {
			...
			if ( $post['images'] ) foreach ( $post['images'] as $image_key => $image ) {
				...
				$attachment_id = CWPIE_Product_Import::process_attachment( $attachment, $image, $post_id );
		...
		if ( ! empty( $post['attributes'] ) && is_array($post['attributes']) ) {
			...
			uasort( $attributes, array( CWPIE_Product_Import, 'attributes_cmp' ) );
		...
		if ( $merging ) {
			CWPIE_Product_Import::add_import_result( 'merged', 'Merge successful', $post_id, $processing_product_title, $processing_product_sku );
			...
			CWPIE_Product_Import::add_import_result( 'imported', 'Import successful', $post_id, $processing_product_title, $processing_product_sku );
	...
	*/
	public function process_attachment( $post, $url, $post_id ) {
		...
		if ( ! strstr( $url, 'http' ) ) {
			...
			$upload = CWPIE_Product_Import::fetch_remote_file( $url, $post );
