 ...
 */
add_filter( 'cron_schedules', 'csv_wc_product_import_cron_intervals_callback');

/**
 * add method to register event to WordPress init
 */
add_action( 'init', 'register_csv_wc_product_import_cron_event');

/**
 * this method will register the cron event
 */
function register_csv_wc_product_import_cron_event() {
	global $wpdb; 
    ...
    foreach( $results as $result ) {
    	...
    	if($current_time>=$cron_time){
	    	if( !wp_next_scheduled( 'csv_wc_product_import_cron_'.$result->cron_id ) ) {
	    		if($result->frequency=='csv_one_time'){
			    	wp_schedule_single_event(time(), 'csv_wc_product_import_cron_'.$result->cron_id);
			    }else{
			    	wp_schedule_event(time(), $result->frequency, 'csv_wc_product_import_cron_'.$result->cron_id);
			    }
		    }
		}    
    }
...
foreach( $results as $result ) {
	...
	if($current_time>=$cron_time){
		 ...
		 */
        add_action('csv_wc_product_import_cron_'.$result->cron_id, 'all_csv_wc_product_import_cron', 10);                
